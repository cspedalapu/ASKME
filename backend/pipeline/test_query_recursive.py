...
chroma_client = chromadb.PersistentClient(path="../output/chroma_store_recursive")
collection = chroma_client.get_or_create_collection(name="document_chunks_recursive")

try:
    collection = chroma_client.get_collection(name="document_chunks_recursive")
...
def query_loop():
    ...
    while True:
        ...
        for i, (doc, meta, dist) in enumerate(zip(results["documents"][0], results["metadatas"][0], results["distances"][0])):
            ...
            print(f"Source: {meta['filename']} | Chunk ID: {meta['chunk_id']} | Tokens: {meta['num_tokens']}")
